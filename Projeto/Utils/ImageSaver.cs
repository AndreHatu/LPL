			...
			using (Bitmap bitmap = (Bitmap)Image.FromStream(file.InputStream))
			{
				...
				if (max > 1000 || bitmap.Width != bitmap.Height)
				{
					...
					using (Bitmap square = new Bitmap(max, max, System.Drawing.Imaging.PixelFormat.Format24bppRgb))
					{
						...
						square.Save(HttpContext.Current.Server.MapPath("~/App_Data/" + id + ".jpg"), System.Drawing.Imaging.ImageFormat.Jpeg);
					}
				}
				else
				{
					bitmap.Save(HttpContext.Current.Server.MapPath("~/App_Data/" + id + ".jpg"), System.Drawing.Imaging.ImageFormat.Jpeg);
				}
			}
		}
	}
